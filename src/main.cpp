...
public:
    ...
    GameObjectFactory(std::string_view str, bool ldm) {
        ...
        do {
            ...
            int opcode = 0;
            if (view[0] == 'k') {
                //std::cout << "balls " << view.substr(2, pos).data() << "\n";
    ...
    void setupEffectGameObject(EffectGameObject* object) {
        ...
        object->m_targetItemID = getValue<int>(80);
    ...
    GameObject* generate() {
        ...
        object->loadGroupsFromString(getValue(57));
        ...
        // fuck you
        //object->customObjectSetup(mainMap);
        ...
        if (type == 0 || type == 21 || type == 25) {
            object->m_rotation = floor(object->m_rotation / 90.0) * 90.0;
        } else if (type == 36) {
...
#define BENCHMARK_END std::cout << std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now()-start).count() << "ns\n";
